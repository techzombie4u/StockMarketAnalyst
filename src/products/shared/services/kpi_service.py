        ...
        try:
            ...
            try:
                ...
                if feature_flags.is_enabled('enable_goahead_triggers'):
                    ...
                    # Create KPI data structure for GoAhead
                    kpi_data = {timeframe: kpis} # Pass the entire kpis dict for more context
                    goahead_decisions = goahead_engine.analyze_kpis_and_decide(kpi_data)

                    logger.info(f"GoAhead analysis complete: {len(goahead_decisions)} decisions generated")
            except Exception as e:
                logger.warning(f"GoAhead analysis failed: {e}")

            return {
                'timeframe': timeframe,
                'metrics': kpis, # Changed to 'metrics' for consistency with the provided snippet
                'timestamp': datetime.now().isoformat(),
                'product': product,
                'status': 'success',
                'goahead_decisions': [asdict(d) if hasattr(d, '__dict__') else d for d in goahead_decisions]
            }
