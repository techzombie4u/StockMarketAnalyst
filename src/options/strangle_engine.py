...
class StrangleEngine:
    ...
    def market_stability_score(self, rv20_pct: float, adx: float, event_flag: str, iv_rank: int) -> int:
        ...
        20%: Gap risk (0 if events, else 1)
    ...
    def dynamic_stop_loss_percent(self, iv_rank: int) -> float:
        """
        Calculate dynamic stop loss as percentage of credit
        Higher IV rank → tighter stop loss
        """
        try:
            cfg = self.config
            SL_low = cfg["default_stop_loss_factor_min"]
            SL_high = cfg["default_stop_loss_factor_max"]
            
            # Scale by IV Rank
            scale = (iv_rank - 50) / 50
            scale = max(-1, min(1, scale))  # Clamp to [-1, +1]
            
            stop_loss_factor = SL_low + (SL_high - SL_low) * max(0, 1 - scale)
            stop_loss_percent_of_credit = stop_loss_factor * 100
            
            return round(stop_loss_percent_of_credit, 1)
    ...
    def enrich_row(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            net_credit_per_lot = row.get('net_credit_per_lot', 0)
            ...
            # Compute all metrics
            enriched['roi_on_margin_percent'] = self.compute_roi_on_margin(
                net_credit_per_lot, lot_size, margin_required
            )
            
            breakout_data = self.monte_carlo_breakout_prob(
                spot, put_strike, call_strike, iv_percent, dte_days
            )
            enriched.update(breakout_data)
            
            enriched['theta_per_day_per_lot'] = self.compute_theta_per_day(
                theta_call, theta_put, lot_size
            )
            
            enriched['max_loss_two_sigma'] = self.stress_max_loss_two_sigma(
                spot, put_strike, call_strike, iv_percent, dte_days, 
                net_credit_per_lot, lot_size
            )
            
            enriched['stop_loss_percent_of_credit'] = self.dynamic_stop_loss_percent(iv_rank)
            
            enriched['market_stability_score'] = self.market_stability_score(
                rv20_pct, adx, event_flag, iv_rank
            )
            
            # Breakeven calculations
            credit_per_share = net_credit_per_lot / lot_size if lot_size > 0 else 0
            enriched['breakeven_lower'] = put_strike - credit_per_share
            enriched['breakeven_upper'] = call_strike + credit_per_share
            
            # Generate verdicts
            enriched['verdict'] = self._generate_verdict(enriched)
            enriched['ai_agent_verdict'] = self._generate_ai_verdict(enriched)
